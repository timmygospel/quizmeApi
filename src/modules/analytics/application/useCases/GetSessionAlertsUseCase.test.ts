import { GetSessionAlertsUseCase } from "./GetSessionAlertsUseCase";
...
function makeRepo(overrides: Partial<IAnalyticsRepository> = {}): IAnalyticsRepository {
    return {
        ...
        getSessionAlerts: jest.fn().mockResolvedValue({ alerts: [] }),
...
describe("GetSessionAlertsUseCase", () => {
    it("passes the caller's effective scope through to the repository", async () => {
        const repo = makeRepo();
        const useCase = new GetSessionAlertsUseCase(repo);
        ...
        expect(repo.getSessionAlerts).toHaveBeenCalledWith("session-1", scope);
    });
});
